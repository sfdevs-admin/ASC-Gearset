...
import B2B_Simple_Product_ID_Label from '@salesforce/label/c.B2B_Simple_Product_ID_Label';
...
export default class SearchProductField extends LightningElement {
     ...
     */
    get label() {
        if( this.productData && this.productData.fields && this.productData.fields.B2B_Is_Simple_Product__c === 'true' && this.displayData?.label === 'Catalog Number' ){
            // console.log('is simple product field');
            // return 'Product Id';
            return B2B_Simple_Product_ID_Label;
        }
     ...
     */
    get value() {
        var labelVal = this.displayData?.value;
        if(labelVal.length > 150){
            // labelVal = labelVal.substring(0,150);
            // labelVal = labelVal + '...';
        }
        return (labelVal ?? '')
            .replace(/&lt;/g, '<')
    ...
    get allContentCustomClasses() {
        ...
        if (!this.isVariationType) {
            ...
            classes.push('normalHeight');
        }

        if( this.displayData?.name === 'Name' ){
            classes.push('productNameText');
            classes.push('slds-m-bottom_small');
        }
        return classes.join(' ');
    }
