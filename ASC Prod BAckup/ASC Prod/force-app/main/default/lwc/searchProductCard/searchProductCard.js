...
export default class SearchProductCard extends LightningElement {
    ...
    onGetProductDetail(res){
        if(res.data){
            this.customProdDetails = res.data;
        }
    }
    ...
    set displayData(data) {
        this._displayData = JSON.parse(JSON.stringify(data));
        if(data.id){
    ...
    get fields() {
        ...
        if( this.customProdDetails && this.customProdDetails.fields && this.customProdDetails.fields.StockKeepingUnit && this.configuration?.fieldConfiguration['StockKeepingUnit']){
            let skuTemp;
            let indexToRemove;
            for(let i = 0; i <= this.displayData?.fields.length; i++){
                if(this.displayData?.fields[i] && this.displayData?.fields[i].name === 'StockKeepingUnit' ){
                    skuTemp = this.displayData?.fields[i];
                }

                if(this.configuration && this.configuration.layout === 'grid' && this.displayData?.fields[i]?.name === 'Name'){
                    indexToRemove = i;  //this.displayData.fields.splice(i, 1);
                }
            }
            if( indexToRemove != undefined ){
                this.displayData.fields.splice(indexToRemove, 1);
            }
            if( !skuTemp ){
                let tempFieldData = {};
                tempFieldData.name = 'StockKeepingUnit';
                tempFieldData.label = 'Catalog Number';
                tempFieldData.tabStoppable = false;
                // tempFieldData.type = undefined;
                tempFieldData.value = this.customProdDetails.fields.StockKeepingUnit;
                let skuVal = this.displayData?.fields.find(obj => obj.label === tempFieldData.label);
                if( !skuVal ){
                    this.displayData?.fields.push(tempFieldData);
                }
            }
        }

        if(this.customProdDetails && this.customProdDetails.fields && this.customProdDetails.fields.CAS__c){
            let tempFieldData = {};
            tempFieldData.name = 'CAS__c';
            tempFieldData.label = 'CAS Number';
            tempFieldData.tabStoppable = false;
            // tempFieldData.type = undefined;
            tempFieldData.value = this.customProdDetails.fields.CAS__c;
            let casVal = this.displayData?.fields.find(obj => obj.label === tempFieldData.label);
            if( !casVal ){
                this.displayData?.fields.push(tempFieldData);
            }
        }
        ...
        return (this.displayData?.fields ?? []).map((field) => {
            if( this.configuration?.layout === 'grid' && this.configuration?.fieldConfiguration[field.name] === 'Name' ){
    ...
    get imageDivCSS(){
        if( this.isGridLayout ){
            //grid view
            return 'imageArea slds-align_absolute-center';
        }else{
            //list view
            return 'imageArealist slds-align_absolute-center';
        }
    }
    ...
    get image() {
        ...
        let img;
        if( this.displayData?.image ){
            img = this.displayData?.image;
        }else if( this.displayData?.defaultImage ){
            img = this.displayData?.defaultImage;
        }
        return {
