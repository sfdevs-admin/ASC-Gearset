...
export function normalizeRefinements(input) {
    let retObj = (input ?? []).map((refinement) => {
...
export function createTreeFromCategory(searchCategoryData, currentPageReference, categoryPath) {
    ...
    if (Object.keys(searchCategoryRep).length > 0) {
        searchCategoryRep.category = Object.assign({}, searchCategoryRep.selectedCategory);
        ...
        tree.selectedCategory = {
            ...
            label: searchCategoryRep.category.label,
            categoryName: searchCategoryRep.category.categoryName,
            items: [],
        };
        tree.selectedCategory.items = searchCategoryRep.category.items?.map((child) => {
            let prodCountVar = child.productCount ?? '';
            let label;
            if( prodCountVar != ''){
                label = `${child.categoryName} (${prodCountVar.toString()})`;
            }else{
...
export function createFacetValueLabel(value, checked = false) {
    // const valueAndProductCount = `${value.name} (${value?.productCount?.toString()})`;
    const valueAndProductCount = `${value.name}`;
    return {
        id: value.id,
...
export function transformInputFacetLabels(facetData, refinements, mruFacet) {
    const facets = facetData.map((facetDataItem) => {
        console.log('transformInputFacetLabels facets-- ', facetDataItem);
