...
import getCookieData from "@salesforce/apex/B2B_CookieConsentService.getCookieData";
import createCookieConsentRecords from "@salesforce/apex/B2B_CookieConsentServiceGuestHelper.createCookieConsentRecords";
import verifyBrowserId from "@salesforce/apex/B2B_CookieConsentService.verifyBrowserId";
import getCookiesToDelete from "@salesforce/apex/B2B_CookieConsentServiceGuestHelper.getCookiesToDelete";

import { loadScript, loadStyle } from "lightning/platformResourceLoader";
import ClientJS from "@salesforce/resourceUrl/clientFingerprint";
...
export default class CookieConsent extends LightningElement {
  ...
  @api rejectButtonLink = '/privacy-statement';
  ...
  async connectedCallback() {
    this.preLoadConsentScript();
    // this.getFingerprint();
  }

  postLoadConsentScript() {
  ...
  async preLoadConsentScript(){
    Promise.all([
      loadScript(this,ClientJS)
    ]).then((res) => {
      this.getFingerprint();
    })
    .catch(error => {
  ...
  getFingerprint(){
    const clientObj = new window.ClientJS(); // this is needed in ascensus for it to work, was not needed in demo org
    this.fingerPrintVal = clientObj.getFingerprint();
    this.postLoadConsentScript();
  }
  ...
  getCookiesFromHead() {
    ...
    let fingerprint = this.fingerPrintVal;
  ...
  get rejectButtonLabelVal(){
    return this.rejectButtonLabel ?? 'Leave Site';
  }

  get necessaryCookieDesc(){
    let val;
    if( this.cookieData.length > 0){
      val = this.cookieData[0].SectionCookies[0]?.Description__c ?? '';
    }else{
      val = 'Cookie Instructions';
    }
    console.log('get necessaryCookieDesc--- '+val);
    return val;
  }
}